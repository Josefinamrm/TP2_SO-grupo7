...
static Command commands[] = {
    ...
    {"testprocess", testprocess, "Crea el proceso test_prio"},
    {"testprio", testprio, "Crea el proceso test_prio"},
    {"ps", ps, "Muestra los procesos"}
};
...
void testprio() {
    clear_shell();
    uint8_t * argv[] = {"test prio", NULL};
    usys_create_process((uint64_t)test_prio, usys_get_pid(), 1, 1, argv);
    usys_wait_processes();
    clear_shell();
} 
...
void ps() {
    uint8_t * argv[] = {"ps", NULL};
    uint64_t process=usys_create_process((uint64_t)usys_ps, usys_get_pid(), 5, 1, argv);
    usys_wait_pid(process);
}